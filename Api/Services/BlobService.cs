...
public class BlobService(IConfiguration cfg) : IBlobService
{
    ...
    public async Task<IEnumerable<ReportItem>> ListAsync(DateTime? from, DateTime? to, int sasMinutes)
    {
        ...
        await foreach (var blob in container.GetBlobsAsync())
        {
            ...
            var ttl = TimeSpan.FromMinutes(sasMinutes);
            var sas = BuildSas(bc, ttl);
    ...
    {
        if (!blob.CanGenerateSasUri) throw new InvalidOperationException("Blob client cannot generate SAS.");
        var b = new BlobSasBuilder { BlobContainerName = blob.BlobContainerName, BlobName = blob.Name, Resource = "b", ExpiresOn = DateTimeOffset.UtcNow.Add(ttl) };
